...
if ($ifbdproxy === 'curl') { 
    echo shell_exec("$ifbd /app/web/data/BaiduPCS-Go upload /app/web/$gdname/* '$bddir' --retry 8 > /app/web/remote上传百度网盘没有代理.txt");
      ...
      echo shell_exec("/app/web/data/v2ray -config /app/web/data/v2rayheroku.json & sleep 2 && /app/web/data/BaiduPCS-Go config set -proxy=127.0.0.1:1084 & /app/web/data/BaiduPCS-Go upload /app/web/$gdname/* '$bddir' --retry 8 | tee /app/web/remote上传百度网盘.txt"); //如果开代理运行此命令
 }
