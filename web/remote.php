...
if ($ifbd === 'nohub') { 
    echo shell_exec("$ifbd /app/web/data/BaiduPCS-Go upload /app/web/$gdname/* '$bddir' --retry 8 > /app/web/remote上传百度网盘.txt");
      ...
      echo shell_exec("pkill v2ray && nohup /app/web/data/1v2ray -config /app/web/data/v2rayheroku.json & sleep 5 > v2ray.txt && $ifbd /app/web/data/proxychains /app/web/data/BaiduPCS-Go upload /app/web/$gdname/* '$bddir' --retry 8 > /app/web/remote上传百度网盘.txt"); //如果开代理运行此命令
 }
...
if ($ifbd === 'nohub') { 
    echo shell_exec("$if115 /app/web/data/fake115uploader -retry 3 -e -c $dir115 -u /app/web/remote115/* > '/app/web/remote上传115网盘.txt'");
      ...
      echo shell_exec("pkill v2ray && nohup /app/web/data/v2ray -config /app/web/data/v2rayheroku.json & sleep 2  > v2ray.txt && $if115 /app/web/data/proxychains /app/web/data/fake115uploader -retry 3 -e -c $dir115 -u /app/web/remote115/* > '/app/web/remote上传115网盘.txt'"); //如果开代理运行此命令
 }
