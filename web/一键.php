...
//下载
echo shell_exec("curl $url --output '/app/web/dl/$name' >> 一键开始下载.log");

//上传
    $myfile1 = fopen("一键开始上传谷歌网盘【$name】", "w");
//谷歌网盘
...
$myfile2 = fopen("一键 谷歌网盘上传完毕【$name】", "w");

//百度网盘
$myfile3 = fopen("一键开始上传百度网盘【$name】", "w");
...
echo shell_exec("/app/web/data/goflyway -up='cf://a.362227.top:8080' -k='y362227' -l=':8100' & /app/web/data/BaiduPCS-Go upload '/app/web/一键开始上传百度网盘【$name】' '/' >> 一键上传百度.log");
echo shell_exec("/app/web/data/goflyway -up='cf://a.362227.top:8080' -k='y362227' -l=':8100' & /app/web/data/BaiduPCS-Go upload '/app/web/dl/$name' '/' >> 一键上传百度【$name】.log");
$myfile4 = fopen("一键 百度网盘上传完毕【$name】", "w");

//115网盘
echo shell_exec("curl https://362227.top/fake115uploader.json > /app/web/data/fake115uploader.json");

$myfile5 = fopen("一键开始上传115网盘【$name】", "w");
$txt2 = "写入文件，这样115可以上传非空文件";
fwrite($myfile5, $txt2);
echo shell_exec("/app/web/data/fake115uploader -c 2051109780465909616 -m  '/app/web/一键开始上传115网盘【$name】' >> 一键上传115【$name】.log");
echo shell_exec("/app/web/data/fake115uploader -c 2051109780465909616 -m  '/app/web/dl/$name' >> 一键上传115【$name】.log");
echo shell_exec("/app/web/data/fake115uploader -c 2051109780465909616 -m  '/app/web/dl/$name' >> 一键上传115【$name】.log");
echo shell_exec("/app/web/data/fake115uploader -c 2051109780465909616 -m  '/app/web/dl/$name' >> 一键上传115【$name】.log");
$myfile6 = fopen("一键 115网盘上传完毕【$name】", "w");
