...
if(file_exists("vimeodl2上传百度网盘.txt")){ 
    ...
    if($Y < 8 || preg_match_all('/.*检测秒传中, 请稍候|准备上传.*/', $lastline, $lastline)) {echo '<caption><h3><font color="#FF0000">正在执行上传任务，请勿添加新链接，否则本任务会被强行取消.</font></h3></caption>';}
}

if(file_exists("vimeodl2下载日志.txt")){ 
    $G=date(filemtime("vimeodl2下载日志.txt"));
    $X = $A- $G;
    if($X < 8) {echo '<caption><h3><font color="#FF0000">正在执行任务，请勿添加新链接，否则本任务会被强行取消</font></h3></caption>';}
}
...
function readableBytes($size0) {
   ...
   while($file=readdir($dir_handle))
   {
   	 if($file!="."&&$file!="..")
   	 {
   	 	...
   	 	echo '<td>'.$num.'</td>';//序号
   	 	echo '<td><a href="/vimeodl2/'.$file.'" target="_blank">'.$file.'</a></td>'; //名称 链接
